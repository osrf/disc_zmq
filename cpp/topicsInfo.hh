...
class TopicsInfo
{
  ...
  public:
    ...
    bool GetCallback(const std::string &_topic, Callback &_cb)
    {
      if (!this->HasTopic(_topic))
        return false;
    ...
    bool GetReqCallback(const std::string &_topic, ReqCallback &_cb)
    {
      if (!this->HasTopic(_topic))
        return false;
    ...
    bool GetRepCallback(const std::string &_topic, RepCallback &_cb)
    {
      if (!this->HasTopic(_topic))
        return false;
    ...
    void SetCallback(const std::string &_topic, const Callback &_cb)
    {
      if (!this->HasTopic(_topic))
      {
        TopicInfo *topicInfo = new TopicInfo();
    ...
    /// \param[in] _topic Topic name.
    /// \param[in] _cb New callback.
    void SetReqCallback(const std::string &_topic, const ReqCallback &_cb)
    {
      if (!this->HasTopic(_topic))
      {
        TopicInfo *topicInfo = new TopicInfo();
    ...
    /// \param[in] _topic Topic name.
    /// \param[in] _cb New callback.
    void SetRepCallback(const std::string &_topic, const RepCallback &_cb)
    {
      if (!this->HasTopic(_topic))
      {
        TopicInfo *topicInfo = new TopicInfo();
